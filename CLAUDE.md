...
- **Status vocabulary** (one source, three layers): `HealthLevel` type in `resource-utils.ts` (`healthy | degraded | alert | unhealthy | neutral | unknown`) → `.status-*` CSS classes (`theme/components.css`) → `<StatusPill>` and `<StatusDot>` typed wrappers (`components/ui/StatusPill.tsx`). All three carry the same six tones; `StatusPill`/`StatusDot` just emit the corresponding `.status-*` class so there is no parallel vocabulary. The `alert` (orange) tier is the intermediate between `degraded` (amber) and `unhealthy` (red) — used for severity gradients (Problems pages, Audit findings, Cert expiry) where the data carries a 3-step urgency that must be visually distinguishable. Use `mapHealthToTone(severityOrHealthString)` to normalize raw API values onto a tone.
