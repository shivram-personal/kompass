...
import {
  ...
  PortForwardPanel,
} from "./components/portforward/PortForwardManager";
import {
  DockProvider,
  BottomDock,
  useDock,
...
import {
  NamespaceSwitcher,
  type NamespaceSwitcherHandle,
} from "./components/NamespaceSwitcher";
...
import {
  ...
  useContextSwitch,
} from "./context/ContextSwitchContext";
...
const CRD_HIDDEN_BY_DEFAULT = new Set([
  "GatewayClass",
  "IngressClass",
  "NodePool",
  "NodeClaim",
      ...
      <div className="flex flex-col items-center gap-4 max-w-md text-center">
        ...
        <div>
          ...
          <p className="text-sm text-theme-text-secondary mt-2">
            Radar is configured with proxy authentication. Access it through
            your organization's auth proxy to authenticate.
          </p>
        </div>
      </div>
    </div>
...
function AppInner() {
  ...
  const { pinned: navRailPinned, togglePinned: toggleNavRailPinned } =
    useNavRailPinned();
  ...
  const chromeless =
    navCustomization.embedded === true && navCustomization.chrome === "none";
  ...
  const getInitialState = () => {
    ...
    return {
      ...
      grouping:
        (searchParams.get("group") as GroupingMode) ||
        (namespaces.length === 0 ? "namespace" : "none"),
    };
  };
  ...
  const setMainView = useCallback(
    (view: ExtendedMainView, params?: Record<string, string>) => {
  ...
  const [namespaces, setNamespaces] = useState<string[]>(
    getInitialState().namespaces,
  );
  // For large clusters: force SSE to reconnect with namespace filter
  const [forceNamespaceFilter, setForceNamespaceFilter] = useState<
    string[] | undefined
  >(undefined);
  const [selectedResource, setSelectedResource] =
    useState<SelectedResource | null>(null);
  const [drawerInitialTab, setDrawerInitialTab] = useState<"detail" | "yaml">(
    "detail",
  );
  const [selectedHelmRelease, setSelectedHelmRelease] =
    useState<SelectedHelmRelease | null>(null);
  const [topologyMode, setTopologyMode] = useState<TopologyMode>(
    getInitialState().topologyMode,
  );
  const [groupingMode, setGroupingMode] = useState<GroupingMode>(
    getInitialState().grouping,
  );
  ...
  const [topologyFocus, setTopologyFocus] = useState<{
    id: string;
    nonce: number;
  } | null>(null);
  ...
  const [displayedTopology, setDisplayedTopology] =
    useState<typeof topology>(null);
  ...
  useEffect(() => {
    const wailsRuntime = (window as unknown as Record<string, unknown>)
      .runtime as
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("radar:open-local-terminal", handler);
  }, [openLocalTerminal]);
  ...
  const selectedResourceKindSlug = selectedResource
    ? kindToPlural(selectedResource.kind).toLowerCase()
    : "";
  const selectedResourceGroup = selectedResource?.group ?? "";
  const selectedResourceRouteMismatch =
    mainView === "resources" &&
    !!selectedResource &&
    (selectedResourceKindSlug !== currentResourceKindSlug ||
      selectedResourceGroup !== currentResourceGroup);
  const resourcesKindRouteChanged =
    mainView === "resources" &&
    prevResourcesKindKeyRef.current !== null &&
    prevResourcesKindKeyRef.current !==
      `${currentResourceGroup}/${currentResourceKindSlug}`;
  const routeSelectedResource =
    resourcesKindRouteChanged && selectedResourceRouteMismatch
      ? null
      : selectedResource;
  ...
  const navigateToResource = useCallback(
    (res: SelectedResource, tab: "detail" | "yaml" = "detail") => {
      const update = () => {
  ...
  const navigateToResourceList = useCallback(
  ...
  const navigateFromIssue = useCallback(
    (resource: SelectedResource) => {
  ...
  useEffect(() => {
    if (navApiResources) initNavigationMap(navApiResources);
  }, [navApiResources]);
  ...
  const views = Object.keys(VIEW_SHORTCUT_KEYS).filter(
    (v): v is ExtendedMainView =>
      VIEW_SHORTCUT_KEYS[v as ExtendedMainView] !== "",
  );
  useRegisterShortcuts([
    ...views.map((view) => ({
      id: `view-${view}`,
    ...
    },
    {
      ...
      handler: () => {
        if (!chromeless) setShowHelp((prev) => !prev);
      },
    },
    {
      id: "command-palette",
      ...
      handler: () => {
        if (showNavRail) omnibarRef.current?.focus();
        else if (!chromeless) setShowCommandPalette(true);
      },
    },
    {
      id: "diagnostics",
  ...
  const hideGroupHeader =
    namespaces.length === 1 && effectiveGroupingMode === "namespace";
  ...
  const {
    ...
    isRetrying,
    updateFromSSE: updateConnectionFromSSE,
  } = useConnection();
  ...
  const fastInvalidationRef = useRef<{
    ...
    changedKinds: new Set(),
    structuralKinds: new Set(),
  ...
  const handleK8sEvent = useCallback(
    (event: K8sEvent) => {
      ...
      const structural =
        event.operation === "add" || event.operation === "delete";
      ...
      if (!structural && slow.timer === null) {
        slow.timer = window.setTimeout(() => {
          ...
          slowInvalidationRef.current = {
            updatedKinds: new Set(),
            timer: null,
          };
        }, 15000);
      }
  ...
  useEffect(
    () => () => {
      if (fastInvalidationRef.current.timer !== null)
        clearTimeout(fastInvalidationRef.current.timer);
      if (slowInvalidationRef.current.timer !== null)
        clearTimeout(slowInvalidationRef.current.timer);
      fastInvalidationRef.current = {
        changedKinds: new Set(),
        structuralKinds: new Set(),
        secretsChanged: false,
  ...
  useEffect(() => {
    const isLarge =
      forceNamespaceFilter !== undefined ||
      topology?.requiresNamespaceFilter === true;
    if (!isLarge) return;
    ...
    setForceNamespaceFilter((prev) => {
      ...
      if (cur.length === next.length && cur.every((ns, i) => ns === next[i]))
        return prev;
      return [...namespaces];
    });
  ...
  const scopeActives = useMemo(
    () => namespaceScope?.actives ?? [],
    [namespaceScope?.actives],
  );
  const namespaceScopeKey = useMemo(
    () => (namespaceScope ? [...scopeActives].sort().join(",") : null),
    [namespaceScope, scopeActives],
  );
  useEffect(() => {
    ...
    const sameAsState =
      sortedScope.length === sortedState.length &&
      sortedScope.every((ns, i) => ns === sortedState[i]);
    ...
    if (!sameAsState) {
      debugNamespaceLog("app:scope-mirror-set-namespaces", {
        nextNamespaces: scopeActives,
      });
  ...
  useEffect(() => {
    ...
    if (mainView === "topology") {
      ...
      if (
        groupingMode !== "none" &&
        (namespaces.length === 0 || groupingMode !== "namespace")
      ) {
  ...
  useEffect(() => {
    ...
    if (urlNamespaces.join(",") !== namespacesKey) {
      debugNamespaceLog("app:url-sync-set-namespaces", {
        nextNamespaces: urlNamespaces,
      });
      ...
      if (namespaceScope) {
        ...
        const same =
          sortedURL.length === sortedScope.length &&
          sortedURL.every((ns, i) => ns === sortedScope[i]);
        if (!same) {
    ...
    if (releaseParam) {
      ...
      if (slashIdx > 0) {
        ...
        setSelectedHelmRelease({
          namespace: ns,
          name,
          storageNamespace: searchParams.get("releaseStorage") || undefined,
        });
      }
    }
  ...
  useEffect(() => {
    if (navigationType !== NavigationType.Pop || mainView !== "resources")
      return;
    const kindFromPath =
      location.pathname.match(/^\/resources\/([^/]+)/)?.[1] ?? "";
    ...
    if (kindFromPath && resourceParam) {
      ...
      const name =
        slashIdx > 0 ? resourceParam.slice(slashIdx + 1) : resourceParam;
  ...
  useEffect(() => {
    ...
    const navigatingToResources =
      mainView === "resources" && prevMainView.current !== "resources";
    const navigatingToHelm =
      mainView === "helm" && prevMainView.current !== "helm";
    prevMainView.current = mainView;
  ...
  const filteredTopology = useMemo((): Topology | null => {
    ...
    const effectiveKinds =
      topologyMode === "fleet" ? FLEET_MODE_KINDS : visibleKinds;

    // Filter by namespace (client-side) and by visible kinds
    const nsSet = namespaces.length > 0 ? new Set(namespaces) : null;
    const filteredNodes = displayedTopology.nodes.filter(
      (node) =>
        effectiveKinds.has(node.kind) &&
        (!nsSet ||
          nsSet.has(node.data.namespace as string) ||
          !(node.data.namespace as string)),
    );
    ...
    const filteredEdges = displayedTopology.edges.filter((edge) => {
      ...
      if (
        edge.skipIfKindVisible &&
        effectiveKinds.has(edge.skipIfKindVisible as NodeKind)
      ) {
        return false;
      }
      return true;
  ...
  const selectedNodeId = useMemo(() => {
    ...
    const match = topology?.nodes.find(
      (n) =>
        ...
        (kindToPlural(n.kind) === selectedResource.kind ||
          n.kind === selectedResource.kind),
    );
    return match?.id;
  ...
  return (
    <PortForwardProvider>
      ...
      <div
        ...
        style={{
          minWidth:
            800 + (showNavRail ? (navRailEffectivePinned ? 176 : 56) : 0),
        }}
      >
        {showNavRail && (
          <PrimaryNavRail
            ...
            accountSlot={
              <UserMenu variant="rail" pinned={navRailEffectivePinned} />
            }
          />
        )}
        ...
        <div className="relative flex flex-col flex-1 min-w-0 h-full">
              ...
              shows it in the header (host may override via brandSlot). */}
                ...
                <div className="flex items-center gap-2">
                  {navCustomization.contextSlot ?? (
                    <ContextSwitcher ref={contextSwitcherRef} />
                  )}
                  ...
                  is the same breakpoint where nav labels appear. */}
                    ...
                    {!connected && (
                      ...
                      >
                        <RefreshCw
                          className={`w-3 h-3 ${isReconnecting ? "animate-spin" : ""}`}
                        />
                      </button>
                    )}
                  </div>
              ...
              {!showNavRail && (
                <div className="md:absolute md:left-1/2 md:-translate-x-1/2 flex items-center gap-0.5 bg-theme-elevated/50 rounded-full p-1 ml-2 md:ml-0">
                  {(
                    ...
                    .filter(
                      ({ view }) => !(view === "checks" && clusterChecksHref),
                    )
              ...
              {showNavRail && (
                <div className="hidden sm:flex flex-1 justify-center min-w-0 px-3">
                  <RadarOmnibar
                    ...
                    onOpenResource={(hit) =>
                      navigateToResourceList(searchHitToSelectedResource(hit))
                    }
                  />
                </div>
              )}
          ...
          {authMe?.authEnabled &&
            !authMe?.username &&
            authMe.authMode === "proxy" && <AuthBarrier authMode="proxy" />}
          {authMe?.authEnabled &&
            !authMe?.username &&
            authMe.authMode === "oidc" && <AuthBarrier authMode="oidc" />}
            ...
            <div className="flex-1 relative bg-theme-base">
              <div className="fixed inset-0 pointer-events-none">
                ...
                <div
                  ...
                  style={{ top: "calc(50% + 34px)" }}
                >
                  <div className="whitespace-nowrap text-[17px] font-semibold tracking-tight text-theme-text-primary">
                    Switching context
                  </div>
                  {targetContext && (
                ...
                <ErrorBoundary>
                  ...
                  {mainView === "home" && (
                    <HomeView
                      ...
                      onNavigateToResourceKind={(kind, apiGroup, filters) => {
                        ...
                        console.debug(
                          "[filters] App.onNavigateToResourceKind:",
                          { kind, apiGroup, filters },
                        );
                        const newParams = new URLSearchParams(searchParams);
                        ...
                        console.debug(
                          "[filters] App.onNavigateToResourceKind: navigating to",
                          targetURL,
                        );
                        navigate({
                          pathname: `/resources/${kind}`,
                      ...
                      onNavigateToResource={(resource) => {
                        ...
                        navigate({
                          pathname: `/resources/${resource.kind}`,
                          search: newParams.toString(),
                        });
                      }}
                    />
                          ...
                          <div className="max-w-md w-full mx-4 text-center">
                            <div className="bg-theme-surface border border-theme-border rounded-xl shadow-lg p-6">
                              ...
                              <p className="text-sm text-theme-text-secondary mb-5">
                                This cluster has too many resources to render
                                the full topology. Select a namespace to
                                explore.
                              </p>
                        ...
                        <>
                          ...
                          <TopologyFilterSidebar
                            ...
                            onEnableHiddenKind={(kind) => {
                              setVisibleKinds((prev) =>
                                new Set(prev).add(kind as NodeKind),
                              );
                              console.log(
                                `[topology] User requested to show hidden kind: ${kind}`,
                              );
                            }}
                          />
                          ...
                          <div className="flex-1 relative">
                            ...
                            <TopologyControls
                              ...
                              showFleetMode={
                                displayedTopology?.nodes?.some((n) =>
                                  FLEET_MODE_KINDS.has(n.kind as NodeKind),
                                ) ?? false
                              }
                            />
                          </div>
                  ...
                  {mainView === "resources" && (
                    <ResourcesView
                      ...
                      onResourceClick={(res) =>
                        res
                          ? navigateToResource(res)
                          : setSelectedResource(null)
                      }
                      onResourceClickYaml={(res) =>
                        navigateToResource(res, "yaml")
                      }
                  ...
                  {mainView === "timeline" && (
                    <TimelineView
                      ...
                      initialViewMode={
                        (searchParams.get("view") as "list" | "swimlane") ||
                        undefined
                      }
                      initialFilter={
          ...
          {!showNavRail && !chromeless && (
            <FloatingButtons
              ...
              onBugReport={() => setShowDiagnostics(true)}
            />
          )}

          {/* Keyboard shortcut help overlay */}
          {helpOverlay.shouldRender && (
            <ShortcutHelpOverlay
          ...
          {/* Diagnostics overlay */}
          {diagnosticsOverlay.shouldRender && (
            <DiagnosticsOverlay
              isOpen={diagnosticsOverlay.isOpen}
    ...
    <div
      ...
      style={{
        ...
        transition: isResizing
          ? "none"
          : `height ${DURATION_DOCK}ms cubic-bezier(0.4, 0, 0.2, 1)`,
      }}
    />
  );
}
...
function FloatingButtons({
  ...
  const btnClass =
    "w-7 h-7 flex items-center justify-center rounded-full bg-theme-elevated/80 hover:bg-theme-hover border border-theme-border-light text-theme-text-tertiary hover:text-theme-text-secondary text-xs font-medium shadow-sm backdrop-blur-sm transition-all";
  return (
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        <Star
          className={`w-3 h-3 hidden xl:block ${starred ? "text-yellow-500 fill-current" : ""}`}
        />
        {starCount !== null && (
          <>
            <span className="w-px h-3 bg-theme-border hidden xl:block" />
            <span className="text-xs tabular-nums hidden xl:inline">
